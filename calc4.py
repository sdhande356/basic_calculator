...
window = tk.Tk()##created one window

window.title('Calculator')##given title

frame = tk.Frame(master=window, bg="green", padx=10)##created a frame in window
frame.pack()
...
def equal():
    ...
    try:
        ...
        y = str(eval(entry.get()))##here it will fetch data inserted by user and after evaluating its result this will get converted into
                            ##string 

        entry.delete(0, tk.END) ##after converting result into string entry widget gets clear

        entry.insert(0, y) #and result appears.

    except:

        tkinter.messagebox.showinfo("Error", "Syntax Error") ##IF there is any error it will show error
...
#We used Lamda Functions here which is
#very useful in building Tkinter GUI applications. They allow us to send multiple data through the callback function
